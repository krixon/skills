...
- **Stale vocabulary** — a documented term or definition the code no longer uses, or now uses to mean something else.
- **Violated decision** — a recorded decision (e.g. an ADR) the implementation no longer honors.
...
- **Dimension** — `docs-drift` (the sub-dimension goes in the title/evidence, e.g. "stale vocabulary term …", "README flag … no longer wired" — never as a new top-level dimension)
