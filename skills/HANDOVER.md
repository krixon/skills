...
**Gate labels.** Two tracker labels are an autonomous run's only halt points: `needs-triage` (a finding awaiting a human's triage decision) and `ready-for-human` (a slice whose implementation carries a judgment a human must make). `auto` never acts on an artifact sitting at either — it leaves the work staged there and stops.
