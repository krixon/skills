...
flowchart LR
    ...
    rev --> ir[/in-review/]
    ir -.-> merge([human merge])
```

`pickup` claims a ready issue (`→ in-progress`) so the loop won't re-grab it, routes by **artifact kind**, then — if the work clears — runs the **review gate** over the branch diff and opens a PR (`→ in-review`) for a human to merge. If it walls, it returns the issue to `needs-triage` with an attempt report — the failure circuit-breaker back at the first diagram's human gate.
...
|---|---|---|---|
| **findings** | `cover` → `capture` → *needs-triage* | an audit | yes — runs to `needs-triage`, stops |
...
They're one axis: *can this proceed without a human?* `pickup`'s autonomy is therefore **conditional on the issue's label** — it runs `ready-for-agent` unattended and stops at `ready-for-human`.
...
So the fully autonomous loop is the right-hand side:

```
schedule ─► auto ─► pickup(next ready-for-agent) ─► tdd / diagnose ─► review gate (code + security) ─► PR ─► human review
```

Everything left of `ready-for-agent` (deciding *what* to build) and the final merge stays human. Everything between can run AFK.
...
- **`auto` (unattended chain)** — delegate every hop. Each skill runs as a subagent; only the handover artifact passes between hops, so the window scales with the number of artifacts, not the volume of work. No visibility cost — nobody watches a live `auto` run.
- **Audits (`cover` / `deepen`)** — fan out `Explore` subagents only when scope is large (> ~25 files), one per area, each returning structured candidates; the main session does the cull (the judgment step, and small). At or below the threshold, explore inline — the reads fit and stay visible.
