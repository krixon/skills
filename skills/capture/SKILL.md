...
Query the issue tracker (per `docs/agents/issue-tracker.md` — `gh issue list` / search) and match each finding against **open** issues by title and `Where`. Drop anything already tracked; never offer the user work that already exists. Flag near-matches so the user decides.
...
For each approved finding, create an issue using the body template in [FINDING-FORMAT.md](FINDING-FORMAT.md). Apply **two labels**: the `needs-triage` state role, and the suggested category role (`bug` / `enhancement`) — front-loading `triage`'s work; `triage` overrides if the guess is wrong. (Canonical role names; the actual label strings are in `docs/agents/triage-labels.md`.)
