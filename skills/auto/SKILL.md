...
- **Report the next wake-up as an absolute time.** A dry poll that schedules the next iteration reports *when* it fires as a wall-clock time, not a bare offset — a reader returning to the session later can't anchor "next iteration in 15m" without knowing when the line was emitted. The agent has only today's date in context and no live clock, so source the time at report time: run `date '+%H:%M'` and add the ladder's offset. Render the result as `next iteration at <HH:MM>` alongside the offset, e.g. `dry — ctx: 156K, next iteration at 14:32 (+15m)`. The ladder values and cadence are unchanged — only how the next wake-up is reported.
