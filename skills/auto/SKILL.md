...
- **Isolate when staging code.** A hop that produces commits (`pickup` and its implement loop) runs on its own branch in a worktree, never the primary working tree — an unattended caller always worktrees, so the run must not disturb the tree you'll return to. See [../../ISOLATION.md](../../ISOLATION.md).
