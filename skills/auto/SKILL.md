...
- **Report context size each iteration.** Surface the parent window after each unit so growth is visible across the drain. Find the transcript by session id, not by mtime — `find ~/.claude/projects -name "$CLAUDE_CODE_SESSION_ID.jsonl"`. Do **not** pick "the newest `*.jsonl`": `ls -t` may be shell-aliased (e.g. to `eza`) and silently return a stale transcript from another session, freezing the number. From that file take the last record carrying `.message.usage` and sum `input_tokens + cache_read_input_tokens + cache_creation_input_tokens` — the figure the status line shows. Report it as e.g. `ctx: 156K`. Rising fast across iterations means state is leaking into the window despite delegation, so the two bullets above are failing — investigate before continuing the drain.
