...
Issue mechanics route through [../ISSUES.md](../ISSUES.md), which selects the tracker binding; PR mechanics live in [../GITHUB.md](../GITHUB.md), and the labels are fixed.
...
- **Derive the next unit in a subagent; return only the decision.** The per-iteration rework scan is dense — it reads every open bot PR's review state — so run it in a subagent that returns one line: `rework <id> kind=changes-requested|thread` for an actionable PR, `new <id>` when none is actionable and an issue is ready, or `dry` when nothing is. The scan output never enters the main window; the loop acts on the decision alone. Sweep rework state across all open bot PRs via the binding in [../GITHUB.md](../GITHUB.md), then read review-thread *bodies* only for the one PR being picked up — that's `pickup`'s classify step (step 1), not the scan. The decision stays identical to `pickup`'s rule: rework before new; a threaded review actionable while any thread is unresolved; a thread-less changes-requested review actionable only when it postdates HEAD.
...
- **Report the next wake-up as an absolute time.** A bare offset ("next iteration in 15m") isn't anchorable later — a reader returning to the session can't tell when the line was emitted. With no live clock, source the time at report time: `date '+%H:%M'` plus the ladder's offset, rendered alongside it as `next iteration at <HH:MM>`, e.g. `dry — ctx: 156K, next iteration at 14:32 (+15m)`. Cadence unchanged — only the reporting.
