...
- **`version`** — versioning. `derive` (range since last `v*` tag → classify → materiality filter → semver bump + grouped notes, per the release policy) and `apply` (worktree, bump, tag, push, teardown). Pure git + semver. Unlike the other two groups, `version` is repo-local — its sole consumer is the repo-local `release` skill (ADR 0005), and releasing *this* plugin means nothing in an installed copy — so it lives at `.claude/skills/release/scripts/version`, outside the distributable, and defaults `--repo` to the repository enclosing the cwd rather than `${CLAUDE_PLUGIN_ROOT}`.
...
- **From the shell** — a developer by hand, or a loop, runs the binary directly. Bare invocation is the *present* shape (read-only — "what would happen"); an explicit verb is the *act* (`bin/land` presents the landable PRs, `bin/land apply` merges them). This is the surface a developer drives by hand, and where a pure command **seeds an interactive session** — `exec claude "<seed>"` built from what *present* emitted (the *Interactive terminal* handoff below) — so a maintainer reaches the guided present→confirm→act flow from a cold shell, not only from inside an existing session. The seed is its own explicit surface (a `drive` verb, or a thin launcher over *present*), never bare invocation: bare stays the side-effect-free *present* shape that a loop fires blind and the in-session wrapper re-runs, and a bare run that exec'd `claude` would break that default and nest an interactive agent inside the session's tool call — the one thing the shell-entry constraint forbids.
...
The human (or a loop) runs a **command**, not a skill. The command does the deterministic work, presents, and where bucket-(3) synthesis is reached, **launches an agent** with a constructed prompt. Four handoff modes — the first two unattended (no human after launch), split by whether a host agent is already running:

- **Subagent** (agent-driven — the default unattended hand-off) — a host agent is already running (`auto`, an in-session drain), so it reaches synthesis by **spawning a subagent in its existing agent loop** (the Task/Agent tool) and consuming the subagent's returned result. This is how the command-launches-agent flips reach synthesis unattended; a subagent's terse return is the channel, no separate process. *An agent cannot shell out to an interactive child, and it has no reason to spawn a second headless `claude` when it can spawn a subagent in-loop* — so an agent-driven run never uses the Headless mode below.
- **Headless** (`claude -p "<brief>" --output-format json --json-schema <outcome-schema>`, **agentless launcher only**) — no host agent at all: a shell or cron entry point starts the work cold. The spawner needs the run's outcome back, and `--json-schema` is the *only* reliable machine-parseable channel: it populates a validated `structured_output` the parent parses (e.g. `{pr_opened, pr_number, walled, reason}`); plain `--output-format json` returns the run's final assistant text in `result`, which the parent would otherwise have to scrape. So the outcome schema is part of the spawn contract, and the constructed brief instructs the agent to conform. Exit code is necessary-not-sufficient (non-zero = failure; zero does not guarantee a usable result). This is a **future slice** — the command-launches-agent flips do not introduce it; an agent-driven `auto` uses the Subagent mode above.
- **Interactive terminal** (`exec claude "<seed>"`, shell-entry only) — the human drives from the first turn. Two cases reach it: a bucket-(3) synthesis entry from a cold shell (a command like `triage`, or an agent-native skill like `design`), and a **pure command seeding the guided present→confirm→act loop** (`land` from a cold shell), which hosts a bucket-(2) decision in chat with no synthesis spawn. Possible *only* because the entry point is the human's shell; an interactive agent cannot be a child of a tool call — which is also why the seed is a distinct verb, never the bare invocation the in-session wrapper re-runs.
- **In-session** — a slash command whose `!`bash-injection runs a *present* command and feeds its output into the turn; the agent renders the menu, the human picks **in chat** (the agent is the interactive surface), the agent then calls an *act* command. The host agent reaches any synthesis directly — it *is* the agent — so no subagent is needed unless the working window needs bounding. No spawn for the hand-off itself.

The command always owns isolation (`worktree create`, claim) *before* spawning, so the agent wakes already on its branch — for a command (like `capture`) whose synthesis writes no code, there is no branch to own, and it spawns (or synthesises in-session) with no worktree.

### Skills that remain, and skills that collapse

- **Collapse to a pure command (no synthesis spawn):** `release`, `reap`, `land`, the work-offering menu. They launch no agent to do model work; a pure command may still *seed* an interactive session from the shell (above) to host its human decision and rendering.
...
**The seam lands per-skill behind a pre-built adapter — not as an atomic repo cutover.** The incoherent state worth avoiding is a *single skill* speaking both vocabularies at once, not the repo having some flipped skills and some prose ones for a while. So the sequencing is: build the adapter first as **pure addition** (`bin/tracker`, `bin/worktree`, and the repo-local `version` script plus their unit tests, naming nothing in any skill — skills keep working on prose throughout), then flip skills to it **one at a time**, stripping the matching prose as the last referent of each binding section goes. The binding files shrink incrementally; no monster PR. Acceptance check, per skill: no `SKILL.md` or command names `gh`/`curl`/a Jira REST path or a git mutation, and none names *both* the adapter and a raw command — the adapter is the only namer within any flipped skill. (This is also how `slice` should cut the work: a foundational adapter-build epic, then one issue per skill flip.)
...
- **Slash-command `!`-injection permission portability** — resolved (#202): an in-session command declares `allowed-tools: Bash(*/bin/<group>:*)` in its frontmatter and invokes the adapter as `${CLAUDE_PLUGIN_ROOT}/bin/<group>`. The permission matcher compares the literal, pre-expansion command string and does no variable expansion of its own, so a leading-glob rule matches the resolved per-install path whether or not `${CLAUDE_PLUGIN_ROOT}` expands — neither a per-install settings rule nor plugin-root expansion in permission strings is needed. An injection matching no allow rule hard-errors rather than prompting, so the frontmatter rule is mandatory.
