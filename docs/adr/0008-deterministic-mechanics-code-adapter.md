...
- **Interactive terminal** (`exec claude "<seed>"`, shell-entry only) — the human drives from the first turn. Two cases reach it: a bucket-(3) command handing into synthesis (triage, design), and a **pure command seeding the guided present→confirm→act loop** (`land` from a cold shell), which hosts a bucket-(2) decision in chat with no synthesis spawn. Possible *only* because the entry point is the human's shell; an interactive agent cannot be a child of a tool call — which is also why the seed is a distinct verb, never the bare invocation the in-session wrapper re-runs.
