...
**What it does.** The dual of `design`: instead of questioning you, it answers questions put *to* the agent. It forms a reasoned answer to each question, one at a time, and converges with you on a shared understanding. Where an agreed answer implies a code change, it returns the change deltas.
