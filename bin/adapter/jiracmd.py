...
The `gh` half of the substrate lives in `ghcmd.py` and the `acli` half in
`aclicmd.py`; this is the Jira REST half. It exists because Python's `urllib`
does not use the OS/corporate trust store, so behind an enterprise TLS-
intercepting proxy a urllib call fails with `CERTIFICATE_VERIFY_FAILED` — while
`curl` (like `acli` and `gh`) honours the OS trust store. The close path's
transition read and write therefore shell out through `curl` rather than the
urllib `/transitions` GET the backend made before.

`run_curl` is the single shell-out point — the backend composes the higher-level
helpers (`request`, `request_json`) over it, and takes the runner as an
injectable seam so unit tests assert on the request built (method, URL, payload)
and the JSON parsed without touching the network.

The security boundary (SECURITY.md): the Basic-auth credential (the Jira email
and api_token) never rides argv. It is written into a `curl` config file fed on
stdin (`curl -K -`), so the token is invisible to `ps`, the same out-of-band
discipline `gh_as_author` keeps for `GH_TOKEN`. The api_token is evaluated from
the credential's command once, materialises only in that config, and is never
logged.
"""
...
def _data_lines(payload: str) -> str:
    ...
    The body is quoted as a single config-file value; embedded backslashes and
    quotes are escaped so the JSON survives curl's config parser intact. The
    backslash pass runs first, so an escape it introduces is never re-escaped by
    the quote pass.
    """
...
def config_for(credential: aclicmd.JiraCredential,
               token_evaluator: Callable[[str], str] | None = None) -> str:
    """The `curl -K -` config for a credential, evaluating its api_token once.

    The api_token is read from the credential's command (the operator's
    indirection) so it materialises only in this config string and never
    persists in the environment. `token_evaluator` is the injectable seam so the
    backend's curl calls are tested offline without spawning the token command.
    """
    token_evaluator = token_evaluator or aclicmd.eval_token
    token = token_evaluator(credential.token_cmd)
    return auth_config(credential.email, token)
