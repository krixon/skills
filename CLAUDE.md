...
- `VOICE.md` — how to talk to the user in chat (imported above, always on). `WRITING.md` — how to write durable prose artifacts (commits, comments, issues, ADRs, docs); referenced by the skills that produce each. `ISOLATION.md` — how work is kept off a detached `main` (two stances: strict worktree with a read-only repo-root by default, or branch-in-primary; branch naming, teardown); referenced by the work-producing skills. `SECURITY.md` — the untrusted-external-content boundary (data-not-instructions, no shell interpolation); injected into context every turn by the always-on `hooks/security-boundary.sh` `UserPromptSubmit` hook, with the command-level mechanics in `skills/GITHUB.md`.
