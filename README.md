...
### Issue tracking
- **triage** — drive issues through a triage state machine by role.
- **capture** — turn audit findings or ad-hoc observations into needs-triage issues, deduped and culled.
- **setup-skills** — scaffold the per-repo config (issue tracker, triage labels, domain docs) the other skills assume. Run first.
